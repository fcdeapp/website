  ...
  const inner = (
    ...
    >
      {previewImages.map((image, index) => {
        ...
        const stackRotate = (index - center) * 1.4;
        const fanX = (index - center) * 18;
        const fanRotate = (index - center) * 5;
    ...
    return (
      <button
        ...
        className={`${styles.imageStackButton} ${
          size === "small"
            ? styles.imageStackButtonSmall
            : styles.imageStackButtonLarge
        }`}
        onClick={onOpen}
      >
        {inner}
  ...
  return (
    <div
      className={`${styles.imageStackDisplay} ${
        size === "small"
          ? styles.imageStackDisplaySmall
          : styles.imageStackDisplayLarge
      }`}
    >
      {inner}
    </div>
...
export default function DiaryClient({ entries }: { entries: DiaryEntry[] }) {
  ...
  const [selectedSlug, setSelectedSlug] = useState<string>(
    sortedEntries[0]?.slug ?? ""
  );

  const selectedEntry =
    sortedEntries.find((entry) => entry.slug === selectedSlug) ??
    sortedEntries[0] ??
    null;

  const monthGroups = useMemo(
    () => buildMonthGroups(sortedEntries),
    [sortedEntries]
  );
          ...
          {showIntro ? (
            <>
              ...
              <p className={styles.sectionLead}>
                아래 달력에서 날짜를 누르면 해당 기록을 바로 확인할 수 있습니다.
              </p>
            </>
          ) : null}
        </div>
        ...
        {entries.length === 0 ? (
          <div className={styles.emptyCard}>
            ...
            <p className={styles.emptyText}>
              <code>src/app/diary/content</code> 폴더에 <code>.txt</code> 파일을
              추가한 뒤 다시 빌드하면 이 페이지에 반영됩니다.
            </p>
          </div>
        ) : (
                        ...
                        entry ? (
                          <button
                            ...
                            className={`${styles.calendarCell} ${
                              ...
                              isActive
                                ? styles.calendarDateButtonActive
                                : ""
                            }`}
              ...
              <section className={styles.selectedPanel}>
                {selectedEntry ? (
                  <>
                    ...
                    <pre className={styles.selectedContent}>
              ...
              {showIntro ? (
                <div className={styles.archiveHead}>
                  ...
                  <p className={styles.archiveText}>
                    카드 목록에서도 원하는 날짜를 눌러 해당 기록으로 이동할 수
                    있습니다.
                  </p>
                </div>
              ) : null}
                  ...
                  <button
                    ...
                    className={`${styles.imageModalThumbButton} ${
                      index === modalState.index
                        ? styles.imageModalThumbButtonActive
                        : ""
                    }`}
                    onClick={() =>
                      setModalState((prev) =>
