...
import ChattingListWeb from "../../components/ChattingListWeb";
...
function ChattingRoomPageInner() {
  ...
  const normalizeAIMessage = (m: any): ChatMessage => ({
    ...
    profileImage: m.role === "assistant" ? "/assets/AbrodyLogo3D.png" : null,
    imageUrl: null,
  });
    ...
    if (isAIChat && chatId) {
      ...
      socket.on("ai:done", (data: { assistantReply: string; choices?: Choice[] }) => {
        ...
        const aiMsg = {
          ...
          profileImage: "/assets/AbrodyLogo3D.png",
        };
  ...
  return (
    <div ref={bubbleRef} className={styles.wrapper}>
      ...
      <button
        ...
        title="Chats"
      >
        <img src="/assets/newChat.png" alt="Chats" width={24} height={24} />
      </button>
      ...
      <ChattingListWeb
        ...
        onSelectChat={(payload) => {
          ...
          // 쿼리 파라미터로 라우팅하여 기존 로직 재사용
          const q = new URLSearchParams();

          switch (payload.type) {
            case "ai":
              q.set("otherUserId", "brody-ai");
              break;
            case "custom-ai":
              q.set("otherUserId", payload.otherUserId);
              if (payload.chatId) q.set("chatId", payload.chatId);
              break;
            case "personal":
              q.set("otherUserId", payload.otherUserId);
              q.set("chatId", payload.chatId);
              break;
            case "group":
