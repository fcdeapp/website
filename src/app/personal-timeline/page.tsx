...
const projectPoint = (code: string) => {
  const city = cities[code];
  const x = ((city.lon + 180) / 360) * 1000;
  const y = ((90 - city.lat) / 180) * 520;
  return { x, y };
};

const makeRoutePath = (route: RouteRecord) => {
  const a = projectPoint(route.from);
  const b = projectPoint(route.to);
  const dx = b.x - a.x;
  const dy = b.y - a.y;
  const distance = Math.hypot(dx, dy);
  const lift = Math.min(190, Math.max(40, distance * 0.22));
  const cx = (a.x + b.x) / 2;
...
export default function PersonalTimeline() {
  ...
  const routeSummary = useMemo(() => {
    const countrySet = new Set(routes.map((r) => themeLabel[r.theme]));
    const citySet = new Set(routes.flatMap((r) => [r.from, r.to]));
    return {
      count: routes.length,
      cityCount: citySet.size,
      themes: Array.from(countrySet).join(" · "),
    };
  }, []);
    ...
    <main className={styles.page}>
      <section className={styles.hero}>
        <div className={styles.heroGrid}>
          <div className={styles.heroCopy}>
            ...
            <h1>이동, 학업, 창업, 건축공학이 하나의 흐름으로 이어진 기록</h1>
            <p className={styles.heroText}>
              항공 이동 기록, 초·중·고·재수·대학입학 타임라인, 2022년 이후 월별 캘린더 기록,
              그리고 2024년 말 이후 대화 기반 프로젝트 관심사를 한 화면의 통합 라이프맵으로 재구성했다.
            </p>
            <div className={styles.heroStats}>
              <article>
                <strong>2011—2021</strong>
                <span>항공 이동 기록</span>
              </article>
              <article>
                <strong>2022—2026</strong>
                <span>월별 학업·프로젝트 기록</span>
              </article>
              <article>
                <strong>2026.06</strong>
                <span>캡스톤·졸업논문 마감</span>
              </article>
            </div>
          </div>
          <div className={styles.heroCard}>
            <div className={styles.heroCardTop}>
              ...
              <strong>Revit · Grasshopper · MIDAS</strong>
            </div>
            <p>
              초기 건축 전공, 군 복무 중 자기계발, 앱 창업 실행기를 거쳐 2026년에는 캡스톤 구조해석과 졸업논문으로 수렴.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className={styles.integratedSection}>
        <div className={styles.sectionHeader}>
          <p className={styles.eyebrow}>Unified Life Map</p>
          <h2>분류를 따로 떼지 않고, 지도 위에 층층이 겹친 통합본</h2>
          <p>
            여행 경로는 지도 위에서 움직이고, 그 위에 학제·전공·군복무·창업·캡스톤이 하나의 레이어처럼 얹힌다.
          </p>
        </div>

        <div className={styles.mapPanel}>
          <svg className={styles.worldMap} viewBox="0 0 1000 520" role="img" aria-label="여행 경로 애니메이션 지도">
            <defs>
            ...
            <rect width="1000" height="520" rx="34" fill="url(#ocean)" />
            <path className={styles.land} d="M110 142 C150 118 218 118 254 148 C292 180 260 228 198 230 C140 232 84 198 92 166 C94 157 100 149 110 142Z" />
            <path className={styles.land} d="M238 286 C300 260 356 284 370 330 C388 388 304 430 238 394 C186 366 174 312 238 286Z" />
            <path className={styles.land} d="M482 150 C548 92 666 106 744 150 C802 182 878 188 910 238 C948 298 852 340 756 310 C690 290 628 322 562 288 C494 254 430 198 482 150Z" />
            <path className={styles.land} d="M662 314 C716 300 770 322 780 366 C790 410 730 442 676 416 C628 392 610 330 662 314Z" />
            <path className={styles.land} d="M812 390 C860 372 912 392 926 430 C940 468 888 492 842 474 C804 458 778 408 812 390Z" />
            <path className={styles.gridLine} d="M0 130 H1000 M0 260 H1000 M0 390 H1000 M250 0 V520 M500 0 V520 M750 0 V520" />
            {routes.map((route, index) => (
              <path
                key={`${route.date}-${route.from}-${route.to}-${index}`}
                d={makeRoutePath(route)}
                className={classNames(
                  styles.routePath,
                  styles[`route-${route.theme}`],
                  index === activeRouteIndex && styles.activeRoute
                )}
                style={{ animationDelay: `${(index % 9) * 0.12}s` }}
              />
            ))}
            {Object.values(cities).map((city) => {
              const pt = projectPoint(city.code);
              return (
                <g key={city.code} className={styles.cityDot} transform={`translate(${pt.x} ${pt.y})`}>
                  <circle r="4.5" />
                  <text x="8" y="4">{city.code}</text>
                </g>
              );
            })}
            <g className={styles.plane}>
              <animateMotion dur="9s" repeatCount="indefinite" rotate="auto">
                <mpath href="#movingRoute" />
              </animateMotion>
              <path d="M0,-8 L20,0 L0,8 L4,1 L-14,1 L-14,-1 L4,-1 Z" />
            </g>
            <path id="movingRoute" d={makeRoutePath(activeRoute)} fill="none" stroke="none" />
          </svg>
      ...
      <section className={styles.timelineSection}>
        <div className={styles.sectionHeader}>
          <p className={styles.eyebrow}>Academic Timeline</p>
          <h2>초등학교부터 대학입학 이후까지</h2>
          <p>학교명은 제공되지 않은 구간이 있어 학제와 사용자가 제시한 기준을 중심으로 정리했다.</p>
        </div>
        <div className={styles.schoolRail}>
          {schoolStages.map((stage) => (
            <article key={stage.period} className={styles.schoolCard}>
              <span>{stage.period}</span>
              <h3>{stage.title}</h3>
              <p>{stage.description}</p>
              <div>
                {stage.markers.map((marker) => (
                  <em key={marker}>{marker}</em>
                ))}
              </div>
            </article>
          ))}
      ...
      <section className={styles.routeSection}>
        <div className={styles.sectionHeaderCompact}>
          <div>
            ...
            <h2>항공 이동 기록</h2>
            <p>금액성·포인트성 정보는 제외하고, 날짜·항공사·구간·여행 맥락만 남겼다.</p>
          </div>
          <div className={styles.routeStats}>
            <article><strong>{routeSummary.count}</strong><span>기록된 구간</span></article>
        ...
        <div className={styles.routeList}>
          {routes.map((route) => (
            <article key={`${route.date}-${route.from}-${route.to}`}>
              <span>{route.date}</span>
              <strong>{route.from} → {route.to}</strong>
              <p>{route.carrier} · {route.label} · {cities[route.from]?.name}에서 {cities[route.to]?.name}까지</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className={styles.monthSection}>
        <div className={styles.sectionHeaderCompact}>
          <div>
            ...
            <h2>2022년 1월부터 2026년 6월까지</h2>
            <p>각 월을 학업·전공·창업·군복무·법인·생활 층위로 다시 나누어 더 촘촘하게 정리했다.</p>
          </div>
